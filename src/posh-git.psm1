#######################################################################
# pwsh -NoProfile -NoExit -WorkingDirectory "$HOME\Repos\posh-git" -Command { $global:PoshGit_InitProps = @{ DisablePoshGitPrompt = $true; UseFunctionCompletion = $true; ShowCompletionErrors = $true; LocalGitVersion = [System.Version]"2.39.2"; LocalGitExtensions = @{ flow = $true } }; Import-Module -Name "$HOME\Repos\posh-git\src\posh-git.psm1"; "--- Got rendered prompt:", $(prompt), $([string]::new("-", 42)) -join "`n" | Write-Host; exit; }
...
# . "$HOME\Repos\EnvConfigs\_tools\domins-profiler.ps1" `
...
__logScopePush "strict-mode-req"
...
if (-not (Test-Path variable:global:LASTEXITCODE)) {
    __logEvent "ensuring LASTEXITCODE set"
